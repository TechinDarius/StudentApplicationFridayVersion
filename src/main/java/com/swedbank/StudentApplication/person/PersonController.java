...
//    @Value("${value.from.file}")
    ...
    ResponseEntity<List<Person>> getAllPersons() {
        ...
        log.info("\nMessage here: Colection size " + list.size());
    ...
    public ResponseEntity<?>  addPersonToGroup(@PathVariable long pid,
    ...
    public ResponseEntity<?> removePersonFromGroup(@PathVariable("pid") long pid,
                                                   @PathVariable long id) throws PersonNotFoundException{
        Person person = service.getById(pid);
        Group group = groupService.findById(id);
